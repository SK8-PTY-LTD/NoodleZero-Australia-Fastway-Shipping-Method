			...
			public function calculate_shipping($package = array()) {
				...
				if ($d_country == "Australia" || $d_country == "AU") {
					if ($d_state == "Victoria" || $d_state == "VIC") {
						...
						// All packages destinationed to New South Wales should be delivered from Sydney. Therefore calculate the delivery fee from SY
						$final_rfcode = "SYD";
								...
								if (is_numeric($tmp_price)) {
									...
									if ($exc > 0) {
										if (is_numeric($exc_price) && !empty($exc_price)) {
											...
											$tmp_price = $tmp_price + $r->excess_label_price_frequent;
										}
									}
